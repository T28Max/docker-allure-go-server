...
func main(){
	...
	for {
		...
		if err != nil { log.Println("redis:", err); time.Sleep(time.Second); continue }
		if len(res) != 2 { continue }
		var job Job
		if err := json.Unmarshal([]byte(res[1]), &job); err != nil { log.Println("bad job:", err); continue }
		if err := process(job, db); err != nil { log.Println("process:", err) }
	}
}
...
func process(j Job, db *sql.DB) error {
	...
	if err := os.MkdirAll(resultsDir, 0o755); err != nil { return err }

	if strings.HasSuffix(j.Path, ".tar.zst") {
		cmd := exec.Command("tar","-I","zstd","-xf", j.Path, "-C", resultsDir)
		out, err := cmd.CombinedOutput()
		if err != nil { return fmt.Errorf("extract tar.zst: %v out=%s", err, string(out)) }
	} else if strings.HasSuffix(j.Path, ".zip") {
		cmd := exec.Command("unzip","-q", j.Path, "-d", workdir)
		out, err := cmd.CombinedOutput()
		if err != nil { return fmt.Errorf("unzip: %v out=%s", err, string(out)) }
		...
		if _, err := os.Stat(filepath.Join(workdir, "allure-results")); err == nil {
			...
			filepath.WalkDir(workdir, func(path string, d os.DirEntry, err error) error {
				if d!=nil && d.IsDir() && filepath.Base(path)=="allure-results" { resultsDir = path; return io.EOF }
				return nil
			})
		}
	} else {
		...
		if strings.HasSuffix(d.Name(), "-result.json") {
			...
			for s.Scan() {
				...
				if strings.Contains(line, "status") {
					counts["total"]++
					if strings.Contains(line, "passed") { counts["passed"]++ }
					if strings.Contains(line, "failed") { counts["failed"]++ }
					break
				}
			}
		}
	...
	if err := os.MkdirAll(reportsBase, 0o755); err != nil { return err }
	reportDir := filepath.Join(reportsBase, j.ID)
	cmd := exec.Command("allure","generate", resultsDir, "-o", reportDir, "--clean")
	out, err := cmd.CombinedOutput()
	if err != nil { log.Printf("allure generate error: %v out=%s", err, string(out)) }
